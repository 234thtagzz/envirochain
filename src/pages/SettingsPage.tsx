...
export default function SettingsPage() {
  ...
  const [blockchainNetwork, setBlockchainNetwork] = useState('hyperledger');
  ...
  const inputClass =
    'w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm placeholder:text-gray-400 focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500';

  const labelClass = 'block text-xs font-medium text-gray-700 mb-1';

  const selectClass =
    'w-full rounded-lg border border-gray-300 bg-white px-3 py-2 text-sm text-gray-900 shadow-sm focus:border-emerald-500 focus:outline-none focus:ring-1 focus:ring-emerald-500';
    ...
    <div className="mx-auto max-w-7xl space-y-6 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <DataCard title="Display Settings" headerAction={<Monitor className="h-4 w-4 text-gray-400" />}>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div>
            ...
            <select value={tempUnit} onChange={(e) => setTempUnit(e.target.value)} className={selectClass}>
      ...
      <DataCard title="API & Integration" headerAction={<Link2 className="h-4 w-4 text-gray-400" />}>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div>
            <label className={labelClass}>Blockchain Network</label>
            <select value={blockchainNetwork} onChange={(e) => setBlockchainNetwork(e.target.value)} className={selectClass}>
      ...
      <DataCard title="About" headerAction={<Info className="h-4 w-4 text-gray-400" />}>
        <div className="space-y-3">
          <div className="flex items-center justify-between text-sm">
            ...
            <span className="font-medium text-gray-900">EnviroChain v0.1.0 - Prototype</span>
          </div>
          <div className="flex items-center justify-between text-sm">
            <span className="text-gray-500">Tech Stack</span>
            <span className="font-medium text-gray-900">React, TypeScript, Tailwind CSS, Recharts, Supabase</span>
          </div>
          <div className="rounded-lg border border-amber-200 bg-amber-50 p-3">
            <p className="text-xs text-amber-700">
              <strong>Disclaimer:</strong> This is a prototype application. All data, blockchain transactions,
              and environmental metrics are simulated for demonstration purposes. This software is not intended
              for production use or regulatory compliance reporting.
            </p>
          </div>
        </div>
      </DataCard>
